...
| 항목 | 내용 |
|---|---|
| 운영 URL | `https://creta.한국` |
| 초기 구축 완료일 | 2026-07-03 |
| 현재 상태 | **운영 중** — GitHub, Vercel, 가비아 도메인, SEO, 네이버 서치어드바이저 등록까지 완료 |

> 문서 상단의 「초기 구축 완료 내역」은 **이미 끝난 작업 기록**입니다.  
> 지금부터 필요한 작업은 콘텐츠 수정·배포·도메인 만료 관리 등 **유지보수**입니다.  
> 전체 변경 기록은 문서 하단 **변경 이력**을 참고하세요.

---

## 2. 현재 운영 기준 (v1.0.0에서 확정됨)

아래 3가지는 v1.0.0 초기 구축 시 확정된 **현재 운영 기준**입니다. (이미 적용 완료)

| 구분 | 운영 기준 |
|---|---|
| Vercel 계정 | **GitHub 계정으로 가입/로그인** |
| 도메인 | **가비아 + 네이버 계정**, `creta.한국` **5년 등록** |
| 네이버 검색 등록 | **네이버 서치어드바이저** 등록 완료 |
...
## 3. 초기 구축 완료 내역 (v1.0.0 · 2026-07-03)

아래는 **이미 완료된** 초기 오픈 작업 기록입니다. 새로 따라 할 필요 없습니다.

| 단계 | 작업 | 상태 |
|---:|---|:---:|
| 1 | GitHub 저장소 구성 및 코드 마이그레이션 | ✅ 완료 |
| 2 | Vercel 가입(GitHub 연동) 및 프로젝트 배포 | ✅ 완료 |
| 3 | GitHub `main` 브랜치 ↔ Vercel 자동 배포 연결 | ✅ 완료 |
| 4 | 가비아 `creta.한국` 도메인 5년 구매 | ✅ 완료 |
| 5 | 가비아 DNS → Vercel 연결 | ✅ 완료 |
| 6 | SEO 설정 (메타태그, robots.txt, sitemap.xml, og-image) | ✅ 완료 |
| 7 | 네이버 서치어드바이저 사이트 등록 및 소유 확인 | ✅ 완료 |
| 8 | 운영 점검 (PC/모바일/문의 연락처) | ✅ 완료 |
...
## 5. 네이버 서치어드바이저 (등록 완료 · 유지보수 가이드)

**v1.0.0(2026-07-03) 기준으로 네이버 서치어드바이저 등록은 완료되었습니다.**

이후 담당자가 주기적으로 확인하거나, 설정 변경 시 참고할 절차입니다.

| 단계 | 작업 | 비고 |
|---:|---|---|
| 1 | [네이버 서치어드바이저](https://searchadvisor.naver.com/) 로그인 | 네이버 계정 사용 |
| 2 | `https://creta.한국` 사이트 등록 | ✅ 등록 완료 |
| 3 | 사이트 소유 확인 (메타태그 방식) | ✅ `index.html` 메타태그 유지 중 |
| 4 | `https://creta.한국/sitemap.xml` 제출 | ✅ 제출 완료 |
| 5 | 웹페이지 수집 요청 | 필요 시 재요청 |
| 6 | 색인/오류 리포트 점검 | 월 1회 권장 |
...
```text
내 작업 PC → GitHub 저장소 → Vercel 자동 배포 → 실제 회사 홈페이지 운영
```
...
별도로 서버를 24시간 켜둘 필요가 없고, FTP 업로드도 필요 없으며, 직접 서버를 관리하지 않아도 됩니다.
...
`npm run dev`를 종료해도 실제 홈페이지는 계속 정상 운영됩니다.

### 수정 후 반영 과정

```text
파일 수정 → npm run dev 로 확인 → git push → Vercel 자동 배포 → 실제 홈페이지 반영
```
...
대부분의 운영 수정은 `src/data/`와 `src/assets/`만 수정하면 처리됩니다.

### 자주 하는 작업

- 전화번호, 이메일, 팩스, 주소 수정
- 회사소개·제품·설비 문구 및 사진 교체
- 협력업체 로고·링크 수정
- SEO 메타태그·공유 이미지 수정 (`index.html`, `public/`)

메뉴 추가나 전체 디자인 변경은 개발자 확인을 권장합니다.
...
## 15. SEO, robots.txt, sitemap, Open Graph
...
| 파일 | 수정 내용 |
|---|---|
| **`src/data/site.js`** | 회사 기본 정보, 연락처, 메뉴 |
| **`src/data/about.js`** | 회사소개, 대표 인사말, 회사 개요 |
| **`src/data/equipment.js`** | 설비 목록, 사양, 사진 |
| **`src/data/products.js`** | 제품소개 항목, 이미지 |
| **`src/data/location.js`** | 주소, 지도 링크, embed |
| **`src/data/partners.js`** | 협력업체, 로고, 링크 |
| **`src/data/history.js`** | 회사 연혁 |
| **`src/data/hero.js`** | 메인 문구, 히어로 이미지 |
| **`index.html`** | SEO 메타태그, OG/Twitter Card, favicon, 네이버 인증 |
| **`src/index.css`** | 전역 색상, 폰트, 기본 스타일 |
| **`public/robots.txt`** | 검색엔진 크롤링 허용, 사이트맵 경로 안내 |
| **`public/sitemap.xml`** | 검색엔진 제출용 URL 목록 |
| **`public/og-image.jpg`** | SNS 공유 미리보기 이미지 (히어로 대표 사진 기반) |
...
## 25. 변경 이력 (Version History)

### v1.0.0 (2026-07-03) — 초기 오픈

- 회사 홈페이지 제작 완료
- GitHub 마이그레이션
- Vercel 배포
- 가비아 도메인(`creta.한국`) 연결 (5년 등록)
- 검색엔진 최적화(SEO) 설정
  - `index.html` 메타태그 (description, OG, Twitter Card, canonical 등)
  - `public/robots.txt` 추가
  - `public/sitemap.xml` 추가
  - `public/og-image.jpg` SNS 공유 이미지 설정
- 네이버 서치어드바이저 등록 완료
